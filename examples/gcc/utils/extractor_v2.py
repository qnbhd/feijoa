...
def parse_optimizers(gcc_toolchain_path):
    try:
        result = executor.execute(f'{gcc_toolchain_path} --help=optimizers -Q', capture=True)
    except executor.ExternalCommandFailed:
        return {}
...
def parse_parameters(gcc_toolchain_path):
    try:
        result = executor.execute(f'{gcc_toolchain_path} --help=params -Q', capture=True)
    except executor.ExternalCommandFailed:
        return {}
...
def get_version(gcc_toolchain_path):
    try:
        ...
        spl = result.split('\n')[0].replace(' ', '_').replace('(', '_').replace(')', '_').replace('.', '_')
